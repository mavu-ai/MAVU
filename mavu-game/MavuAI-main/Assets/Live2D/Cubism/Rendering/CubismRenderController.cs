...
namespace Live2D.Cubism.Rendering
{
    ...
    public sealed class CubismRenderController : MonoBehaviour, ICubismUpdatable
    {
        ...
        void OnValidate()
        {
            ...
            {
                renderController.TryInitializeRenderers(); // Force init
                // Симулируем change mode для refresh
                var originalMode = renderController.SortingMode;
                renderController.SortingMode = CubismSortingMode.FrontToBackZ; // Или твой FrontToBack
                renderController.SortingMode = originalMode; // Возврат, чтобы триггерить update
            }
        }
